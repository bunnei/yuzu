...
void PageHeap::Initialize(VAddr address, std::size_t size, std::size_t metadata_size,
    ...
    heap_size = size;
    num_blocks = num_block_shifts;

    // Setup bitmaps
    metadata.resize(metadata_size / sizeof(u64));
    u64* cur_bitmap_storage{metadata.data()};
    for (std::size_t i{}; i < num_block_shifts; i++) {
        const std::size_t cur_block_shift{block_shifts[i]};
        const std::size_t next_block_shift{(i != num_block_shifts - 1) ? block_shifts[i + 1] : 0};
...
VAddr PageHeap::AllocateBlock(s32 index) {
    ...
    for (s32 i{index}; i < static_cast<s32>(num_blocks); i++) {
...
void PageHeap::Free(VAddr addr, std::size_t num_pages) {
    ...
    s32 big_index{static_cast<s32>(num_blocks) - 1};
